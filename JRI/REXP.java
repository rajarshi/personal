...
public class REXP {
	...
	public REXP(Rengine re, long exp, boolean convert) {
	    ...
	    //long attrsl = re.rniAttrs(xp);
	    //System.out.println("attrsl = "+attrsl);
	...
	REXP dim = null;
	long attrexp = eng.rniGetAttr(xp, "dim");
	if (attrexp == 0) {
	    return null;
	} else {
	    dim = new REXP(eng, attrexp);
	}

	//if (Xt != XT_ARRAY_DOUBLE || attr == null || attr.Xt != XT_ARRAY_INT)
	//    return null;

	if (Xt != XT_ARRAY_DOUBLE || dim == null || dim.Xt != XT_ARRAY_INT)
	    return null;
