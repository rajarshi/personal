...
//import org.openscience.cdk.qsar.model.R2.*;
...
public class jri {
  ...
  public static void main(String[] args) {
    try {
      ...
      REXP newREXP = new REXP(REXP.XT_INT, new Integer(1));
      System.out.println("newREXP = "+newREXP);
      System.out.println("newREXP.rType = "+newREXP.rtype);
      re.assign("anInt", new int[] {1234});

      // set an int[]
      re.assign("anIntArr", new int[] {1,2,3,4});
      REXP ret = re.eval("anIntArr");
      System.out.println("ret = "+ret);
      System.out.println("re.toString() = "+ret.toString());

      // get the value of the int we set previously
      ret = re.eval("anInt");
      System.out.println("re.eval(\"anInt\") = "+ret);
      System.out.println(ret.toString());

      // what do we get back from an assignment?
      ret = re.eval("x <- 2");
      System.out.println(ret);

      // try autoconverting a matrix object
      System.out.println("\nTrying to convert a matrix");
      REXP rexp = re.eval("matrix(c(1,2,3,4,5,6), nrow=2, ncol=3)");
      System.out.println("rexp = "+rexp+" type = "+REXP.xtName(rexp.getType()));
      double[][] d = rexp.asDoubleMatrix();
      System.out.println("d = "+d);
      System.out.println("nrow = "+d.length+" ncol = "+d[0].length);

      // get a complex object, such as lm
      /*
      System.out.println("\n\nWorking with an lm object");
      ret = re.eval("load(\"lm.Rda\")");
      ret = re.eval("m");
	...
	System.out.println("\ncoefficients: ");
	double[] coeff = list.at("coefficients").asDoubleArray();
	for (double c : coeff) System.out.println("  "+c);
...
/*
  public static void testLM() {
    ...
    try {
      ...
      LinearRegressionModel lm = new LinearRegressionModel(x,y);
      lm.build();
      
      double[] coeff = lm.getCoefficients();
      for (double c : coeff) {
	System.out.print(c+" ");
      }
      System.out.println("\n");

      int dfresidual = lm.getDfResidual();
      System.out.println("df.residual = "+dfresidual);

      LinearRegressionModel lm2 = new LinearRegressionModel();
      lm2.loadModel("lm.Rda");
      System.out.println("model name = "+lm2.getModelName());
      coeff = lm2.getCoefficients();
      for (double c : coeff) {
	System.out.print(c+" ");
      }
      System.out.println("\n");

      System.out.println("Testing new predictions");
	...
	{ new Double(3.4541825491), new Double(9.78038580407), new Double(3.58954097059) }
      ...
      lm.setParameters("newdata", newx);
      lm.setParameters("interval", "confidence");
      lm.predict();
      
      lm.saveModel(lm.getModelName(), "junk.Rda");
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public static void testCNN() {
    System.out.println("\n\n-----------------------------\nTesting CNNRegressionModel\n-----------------------------");
    try {
      CNNRegressionModel cnnrm = new CNNRegressionModel();
      cnnrm.loadModel("cnn2.Rda");
      System.out.println("model name = "+cnnrm.getModelName());

      Rengine re = cnnrm.getRengine();
      double[][] d = re.eval("matrix(c(1,2,3,4,5,6),2,3)").asDoubleMatrix();
      System.out.println("d = "+d);

      RList mo = cnnrm.getModelObject();
      System.out.println("model object = "+mo);
      REXP rexp = mo.at("fitted.values");
      System.out.println("rexp Xt = "+REXP.xtName(rexp.getType()));

      double[] fv = mo.at("fitted.values").asDoubleArray();
      System.out.println("fitted.values = "+fv);

      double[][] fitted = cnnrm.getFittedValues();
      if (fitted == null) {
	System.out.println("fitted was null");
      } else {
	System.out.println(fitted.length+" fitted values for "+fitted[0].length+" vars"); 
      }
      double[] n = cnnrm.getN();
